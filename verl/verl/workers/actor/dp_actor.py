...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(
        self, micro_batch, temperature, calculate_entropy=False, top_k=0, student_top_k_ids=None, calculate_logsumexp=False
        ...
        with torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                need_logits = top_k > 0 or calculate_logsumexp
                ...
                if self.use_fused_kernels and not need_logits:
                    ...
                    if calculate_logsumexp:
                        logsumexp_rmpad = torch.logsumexp(logits_rmpad, dim=-1, keepdim=True)  # [total_nnz, 1]
                    ...
                    if need_topk:
                        ...
                        if calculate_logsumexp:
                            ...
                            topk_logsumexp = torch.logsumexp(topk_logits, dim=-1, keepdim=True)
                ...
                if self.use_ulysses_sp:
                    ...
                    if calculate_logsumexp and logsumexp_rmpad is not None:
                        logsumexp_rmpad = gather_outputs_and_unpad(
                            logsumexp_rmpad,
                            gather_dim=0,
                            unpad_dim=0,
                            padding_size=pad_size,
                    ...
                    if top_k > 0:
                         ...
                         if calculate_logsumexp and logsumexp_rmpad is not None:
                            topk_logsumexp = gather_outputs_and_unpad(
                                topk_logsumexp,
                                gather_dim=0,
                                unpad_dim=0,
                                padding_size=pad_size,
                ...
                if calculate_logsumexp and logsumexp_rmpad is not None:
                    full_logsumexp = pad_input(
                        hidden_states=logsumexp_rmpad,
                        indices=indices,
                        batch=batch_size,
                        seqlen=seqlen,
                    )
                ...
                if top_k > 0:
                    ...
                    if calculate_logsumexp and topk_logsumexp is not None:
                        full_topk_logsumexp = pad_input(
                            hidden_states=topk_logsumexp,
                            indices=indices,
                            batch=batch_size,
                            seqlen=seqlen,
                        )
                ...
                if calculate_logsumexp and logsumexp_rmpad is not None:
                    logsumexp = full_logsumexp[:, -response_length - 1 : -1, :]  # [B, L, 1]
                
                if top_k > 0:
                    topk_ids = full_topk_ids[:, -response_length - 1 : -1, :]
                    topk_log_probs = full_topk_log_probs[:, -response_length - 1 : -1, :]
                    if calculate_logsumexp and logsumexp_rmpad is not None:
                        topk_logsumexp = full_topk_logsumexp[:, -response_length - 1 : -1, :]
                ...
                if self.use_fused_kernels and not need_logits:
                    ...
                    if calculate_logsumexp:
                        logsumexp = torch.logsumexp(logits, dim=-1, keepdim=True)  # [B, L, 1]
                    # Optimization: when top_k > 0, compute log_softmax once and gather both
            ...
            return entropy, log_probs, topk_ids, topk_log_probs, logsumexp, topk_logsumexp
    ...
    def compute_distillation_reward(self, data: DataProto) -> DataProto:
        ...
        top_k = data.meta_info.get("reverse_kl_top_k", 0)
        ...
        S_logZ = data.batch["student_logsumexp"].to(device)
        T_on_S = data.batch["teacher_on_student_log_probs"].to(device)
        T_logZ = data.batch["teacher_logsumexp"].to(device)  # [B, L, 1]
        ...
        # elif strategy == "only_stu_kl":
        #     valid_mask = torch.ones_like(S_logp, dtype=torch.bool)
        #     norm_weights = compute_reward_weights(
        #         S_logp, T_on_S, valid_mask, reward_weight_mode
        #     )

        #     S_logits_on_S = S_logp + S_logZ
        #     T_logits_on_S = T_on_S + T_logZ

        #     kl_term_1_student_logit = norm_weights * S_logits_on_S
        #     kl_term_2_teacher_logit = -norm_weights * T_logits_on_S
        #     kl_term_3_student_logZ = -norm_weights * S_logZ
        #     kl_term_4_teacher_logZ = norm_weights * T_logZ
        #     kl_val = kl_term_1_student_logit+ kl_term_2_teacher_logit+ 2.0*kl_term_3_student_logZ + kl_term_4_teacher_logZ
    ...
    @GPUMemoryLogger(role="dp actor", logger=logger)
    def compute_log_prob(self, data: DataProto, calculate_entropy=False, calculate_logsumexp=False) -> torch.Tensor:
        ...
        for micro_batch in micro_batches:
            ...
            model_inputs = {**micro_batch.batch, **micro_batch.non_tensor_batch}
            with torch.no_grad():
                entropy, log_probs, topk_ids, topk_log_probs, logsumexp, topk_logsumexp = self._forward_micro_batch(
                    model_inputs, temperature=temperature, calculate_entropy=calculate_entropy, top_k=top_k, calculate_logsumexp=calculate_logsumexp
                )
            # Keep on GPU to avoid expensive CPU-GPU transfer for large top-k
            ...
            if calculate_logsumexp and logsumexp is not None:
                logsumexp_lst.append(logsumexp)
            if top_k > 0:
                ...
                topk_log_probs_lst.append(topk_log_probs)
                if calculate_logsumexp and topk_logsumexp is not None:
                    topk_logsumexp_lst.append(topk_logsumexp)

        log_probs = torch.concat(log_probs_lst, dim=0)
        entropys = None
        if calculate_entropy:
            entropys = torch.concat(entropy_lst, dim=0)
        logsumexp_tensor = None
        topk_logsumexp_tensor = None
        if calculate_logsumexp:
            logsumexp_tensor = torch.concat(logsumexp_lst, dim=0)
        ...
        if top_k > 0:
            ...
            topk_log_probs_tensor = torch.concat(topk_log_probs_lst, dim=0)
            if calculate_logsumexp and topk_logsumexp_lst:
                topk_logsumexp_tensor = torch.concat(topk_logsumexp_lst, dim=0)

        if use_dynamic_bsz:
            log_probs = restore_dynamic_batch(log_probs, batch_idx_list)
            if calculate_entropy:
                entropys = restore_dynamic_batch(entropys, batch_idx_list)
            if logsumexp_tensor is not None:
                logsumexp_tensor = restore_dynamic_batch(logsumexp_tensor, batch_idx_list)
            if top_k > 0:
                topk_ids_tensor = restore_dynamic_batch(topk_ids_tensor, batch_idx_list)
                topk_log_probs_tensor = restore_dynamic_batch(topk_log_probs_tensor, batch_idx_list)
                if topk_logsumexp_tensor is not None:
                    topk_logsumexp_tensor = restore_dynamic_batch(topk_logsumexp_tensor, batch_idx_list)

        return log_probs, entropys, topk_ids_tensor, topk_log_probs_tensor, logsumexp_tensor, topk_logsumexp_tensor
    ...
    def update_policy(self, data: DataProto):
        ...
        if "teacher_logsumexp" in data.batch.keys():
            select_keys.append("teacher_logsumexp")
        # EOPD Forward-KL tensors
        if "forward_teacher_top_k_ids" in data.batch.keys():
            select_keys.append("forward_teacher_top_k_ids")

        if "forward_teacher_top_k_log_probs" in data.batch.keys():
            select_keys.append("forward_teacher_top_k_log_probs")

        if "teacher_entropy" in data.batch.keys() and "forward_teacher_top_k_ids" in data.batch.keys():
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    # current_student_logsumexp = None
                    # if advantages.dim() == 3:
                    ...
                    #     entropy, _, _, topk_log_probs, current_student_logsumexp, _ = self._forward_micro_batch(
                    #         model_inputs, temperature=temperature, calculate_entropy=calculate_entropy,
                    #         top_k=top_k, student_top_k_ids=student_top_k_ids, calculate_logsumexp=True
                    #     )
                    #     log_prob_for_loss = topk_log_probs
                        
                    # else:
                    #     _, log_prob, *_, _, _ = self._forward_micro_batch(
                    #         model_inputs, temperature=temperature, calculate_entropy=calculate_entropy
                    #     )
                    #     log_prob_for_loss = log_prob
                    current_student_logsumexp = None
                    forward_student_log_probs = None

                    # ------------------------------------------------------------
                    # 1. Reverse-KL IDs
                    # ------------------------------------------------------------
                    reverse_target_ids = None
                    reverse_width = 0

                    if advantages.dim() == 3:
                        # union/only_tch use union_top_k_ids
                        if "union_top_k_ids" in model_inputs:
                            reverse_target_ids = model_inputs["union_top_k_ids"]
                        # only_stu/intersection use student Top-K
                        elif "student_top_k_ids" in model_inputs:
                            reverse_target_ids = model_inputs["student_top_k_ids"]

                        reverse_width = reverse_target_ids.shape[-1]
                        if reverse_width != advantages.shape[-1]:
                            raise RuntimeError(
                                f"reverse id width ({reverse_width}) "
                                f"!= advantages width "
                    ...
                    # 2. Forward-KL IDs
                    #
                    # Always TEACHER Top-K and independent of reverse support.
                    # ------------------------------------------------------------
                    forward_target_ids = model_inputs.get("forward_teacher_top_k_ids", None)
                    forward_width = forward_target_ids.shape[-1] if forward_target_ids is not None  else 0

                    # ------------------------------------------------------------
                    # 3. Combine ID sets and do one actor forward
                    # ------------------------------------------------------------
                    gather_parts = []

                    if reverse_target_ids is not None:
                        gather_parts.append(reverse_target_ids)
                    if forward_target_ids is not None:
                        gather_parts.append(forward_target_ids)


                    if gather_parts:
                        if len(gather_parts) == 1:
                            gather_ids = gather_parts[0]
                        else:
                            gather_ids = torch.cat(gather_parts, dim=-1)

                        gather_k = gather_ids.shape[-1]
                        entropy, sampled_log_prob, _, gathered_log_probs, current_student_logsumexp,  _ = self._forward_micro_batch(
                            model_inputs, temperature=temperature, calculate_entropy=calculate_entropy, top_k=gather_k,
                            student_top_k_ids=gather_ids, calculate_logsumexp=True)
                        # --------------------------------------------------------
                        # Split reverse and forward portions
                        # --------------------------------------------------------
                        offset = 0
                        if reverse_target_ids is not None:
                            reverse_log_probs = gathered_log_probs[...,:reverse_width]
                            offset += reverse_width
                            log_prob_for_loss = reverse_log_probs
                        else:
                            log_prob_for_loss = sampled_log_prob

                        if forward_target_ids is not None:
                            forward_student_log_probs = gathered_log_probs[..., offset : offset + forward_width]
                    else:
                        entropy, sampled_log_prob, *_, _, _ = self._forward_micro_batch(model_inputs, temperature=temperature, calculate_entropy=calculate_entropy)
                        log_prob_for_loss = sampled_log_prob
