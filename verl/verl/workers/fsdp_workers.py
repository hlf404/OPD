...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        data.meta_info["top_k"] = self.config.rollout.get("reverse_kl_top_k", 0)
        # data.meta_info["top_p"] = 1.0
        # print("log_prob_top_k", data.meta_info["top_k"])
        # perform recompute log_prob
        with self.ulysses_sharding_manager:
            with adapter_ctx:
                output, entropys, topk_ids, topk_log_probs, logsumexp, topk_logsumexp = self.actor.compute_log_prob(data=data, calculate_entropy=True, calculate_logsumexp=True)
            ...
            if logsumexp is not None:
                tensors["student_logsumexp"] = logsumexp
            if topk_logsumexp is not None:
                tensors["student_top_k_logsumexp"] = topk_logsumexp
    ...
    def compute_ref_log_prob(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            output, _, _, _, _,_ = self.ref_policy.compute_log_prob(data=data, calculate_entropy=False)
...
class RewardModelWorker(Worker, DistProfilerExtension):
    ...
    def _compute_teacher_top_k_log_probs(self, logits, student_ids, top_k, strategy="only_stu", chunk_size=1024):
        ...
        for start in range(0, n_samples, chunk_size):
            ...
            t_top_k_logsumexp = torch.logsumexp(t_logits, dim=-1, keepdim=True)


            teacher_top_k_ids_list.append(t_ids)
            teacher_top_k_log_probs_list.append(t_log_probs_top_k)
            teacher_top_k_logsumexp_list.append(t_top_k_logsumexp)
        ...
        return (torch.cat(results, dim=0), torch.cat(valid_counts_list, dim=0), torch.cat(overlap_counts_list, dim=0),
                ...
                torch.cat(teacher_in_student_list, dim=0),torch.cat(teacher_top_k_logsumexp_list, dim=0))
    ...
    def _forward_micro_batch(self, micro_batch, student_top_k_ids=None, compute_entropy=False, reverse_top_k=0, forward_top_k=0, strategy="only_stu", teacher_temperature=1.0):    
        ...
        with torch.no_grad(), torch.autocast(device_type=get_device_name(), dtype=torch.bfloat16):
            ...
            teacher_logsumexp = None
            teacher_top_k_logsumexp = None
            forward_teacher_top_k_ids = None
            forward_teacher_top_k_log_probs = None
            need_logits = student_top_k_ids is not None or compute_entropy or reverse_top_k > 0 or forward_top_k > 0
            ...
            if self.use_remove_padding:
                ...
                local_teacher_logsumexp = None
                local_teacher_top_k_logsumexp = None
                local_forward_teacher_top_k_ids = None
                local_forward_teacher_top_k_log_probs = None
                ...
                if self.use_fused_kernels and not need_logits:
                    ...
                    local_teacher_logsumexp = torch.logsumexp(logits_rmpad, dim=-1, keepdim=True)  # [total_nnz, 1]
                    # ============================================================
                    # Forward-KL teacher Top-K.
                    # Completely independent from reverse_top_k.
                    # ============================================================

                    if forward_top_k > 0:
                        fwd_teacher_logits, local_forward_teacher_top_k_ids = torch.topk(logits_rmpad, k=forward_top_k, dim=-1)
                        local_forward_teacher_top_k_log_probs = fwd_teacher_logits - local_teacher_logsumexp
                    ...
                    if student_top_k_ids is not None:
                         ...
                         if reverse_top_k > 0:
                            ...
                            local_top_k_log_probs_on_student_ids, local_valid_counts, local_overlap_mask, local_teacher_top_k_ids, local_teacher_top_k_log_probs, local_teacher_in_student_mask, local_teacher_top_k_logsumexp = self._compute_teacher_top_k_log_probs(
                                logits=logits_rmpad,
                ...
                if self.use_ulysses_sp:
                    ...
                    if local_teacher_top_k_logsumexp is not None:
                        full_teacher_top_k_logsumexp = gather_outputs_and_unpad(
                            local_hidden_states=local_teacher_top_k_logsumexp,
                            indices=indices,
                            batch=batch_size,
                            seqlen=seqlen,
                            sp_size=self.ulysses_sequence_parallel_size,
                    ...
                    if local_teacher_logsumexp is not None:
                        full_teacher_logsumexp = gather_outputs_and_unpad(
                            local_hidden_states=local_teacher_logsumexp,
                            indices=indices,
                            batch=batch_size,
                            seqlen=seqlen,
                            sp_size=self.ulysses_sequence_parallel_size,
                    ...
                    if local_forward_teacher_top_k_ids is not None:
                        full_forward_teacher_top_k_ids = gather_outputs_and_unpad(
                            local_hidden_states=local_forward_teacher_top_k_ids,
                            indices=indices,
                            batch=batch_size,
                            seqlen=seqlen,
                            sp_size=self.ulysses_sequence_parallel_size,
                    ...
                    if local_forward_teacher_top_k_log_probs is not None:
                        full_forward_teacher_top_k_log_probs = gather_outputs_and_unpad(
                            local_hidden_states=local_forward_teacher_top_k_log_probs,
                            indices=indices,
                            batch=batch_size,
                            seqlen=seqlen,
                            sp_size=self.ulysses_sequence_parallel_size,
                    ...
                    if local_teacher_top_k_logsumexp is not None:
                        full_teacher_top_k_logsumexp = pad_input(
                            hidden_states=local_teacher_top_k_logsumexp,
                            indices=indices,
                            batch=batch_size,
                            seqlen=seqlen,
                        )
                    if local_teacher_logsumexp is not None:
                        full_teacher_logsumexp = pad_input(
                            hidden_states=local_teacher_logsumexp,
                            indices=indices,
                            batch=batch_size,
                            seqlen=seqlen,
                        )
                    if local_forward_teacher_top_k_ids is not None:
                        full_forward_teacher_top_k_ids = pad_input(
                            hidden_states=local_forward_teacher_top_k_ids,
                            indices=indices,
                            batch=batch_size,
                            seqlen=seqlen,
                        )
                    ...
                    if local_forward_teacher_top_k_log_probs is not None:
                        full_forward_teacher_top_k_log_probs = pad_input(
                            hidden_states=local_forward_teacher_top_k_log_probs,
                            indices=indices,
                            batch=batch_size,
                            seqlen=seqlen,
                        )
                ...
                if local_teacher_top_k_logsumexp is not None:
                    teacher_top_k_logsumexp = full_teacher_top_k_logsumexp[:, -response_length - 1 : -1, :, ]
                if local_teacher_logsumexp is not None:
                    teacher_logsumexp = full_teacher_logsumexp[:, -response_length - 1 : -1, :]
                if local_forward_teacher_top_k_ids is not None:
                    forward_teacher_top_k_ids = full_forward_teacher_top_k_ids[:, -response_length - 1 : -1, :]
                if local_forward_teacher_top_k_log_probs is not None:
                    forward_teacher_top_k_log_probs = full_forward_teacher_top_k_log_probs[:, -response_length - 1 : -1, :]
            else:
                output = self.reward_module(
                    input_ids=input_ids,
                ...
                if self.use_fused_kernels and not need_logits:
                    ...
                    teacher_logsumexp = torch.logsumexp(rm_logits_resp, dim=-1, keepdim=True)  # [B, L, 1]

                    # ============================================================
                    # Independent teacher Top-K for Forward KL
                    # ============================================================

                    if forward_top_k > 0:
                        fwd_teacher_logits, forward_teacher_top_k_ids = torch.topk(rm_logits_resp, k=forward_top_k, dim=-1)
                        forward_teacher_top_k_log_probs = fwd_teacher_logits - teacher_logsumexp
                    ...
                    if student_top_k_ids is not None:
                        if reverse_top_k > 0:
                            ...
                            flat_on_student_log_probs, flat_counts, flat_overlap_mask, flat_teacher_top_k_ids, flat_teacher_top_k_log_probs, flat_teacher_in_student, flat_teacher_top_k_logsumexp = self._compute_teacher_top_k_log_probs(
                                logits=flat_logits,
                            ...
                            teacher_top_k_ids = flat_teacher_top_k_ids.view(original_shape[0], original_shape[1], reverse_top_k)
                            teacher_top_k_log_probs = flat_teacher_top_k_log_probs.view(original_shape[0], original_shape[1], reverse_top_k)
                            teacher_top_k_logsumexp = flat_teacher_top_k_logsumexp.view(original_shape[0], original_shape[1], 1)
                            teacher_in_student_mask = flat_teacher_in_student.view(original_shape[0], original_shape[1], reverse_top_k)
            ...
            return rm_log_probs, teacher_on_student_log_probs, teacher_top_k_ids, teacher_top_k_log_probs, teacher_entropy, teacher_valid_counts, teacher_overlap_mask, teacher_in_student_mask, teacher_logsumexp, teacher_top_k_logsumexp, forward_teacher_top_k_ids, forward_teacher_top_k_log_probs
    ...
    def compute_rm_score(self, data: DataProto, kl_estimator="k1"):
        ...
        with self.ulysses_sharding_manager:
            ...
            reverse_top_k = data.meta_info.get("reverse_kl_top_k", self.config.get("reverse_kl_top_k", 0))
            forward_top_k = data.meta_info.get("forward_kl_top_k", self.config.get("forward_kl_top_k", 0))
            ...
            output_teacher_logsumexp = []
            output_teacher_top_k_logsumexp = []
            output_forward_teacher_top_k_ids = []
            output_forward_teacher_top_k_logp = []
            ...
            for micro_batch in micro_batches:
                ...
                teacher_logp_batch, teacher_on_student_logp_batch, teacher_top_k_ids_batch, teacher_top_k_logp_teacher_batch, teacher_entropy_batch, teacher_valid_counts_batch, teacher_overlap_mask_batch, teacher_in_student_mask_batch, teacher_logsumexp_batch, teacher_top_k_logsumexp_batch, forward_teacher_top_k_ids_batch, forward_teacher_top_k_logp_batch = self._forward_micro_batch(
                    micro_batch, 
                ...
                if teacher_logsumexp_batch is not None:
                    output_teacher_logsumexp.append(teacher_logsumexp_batch)
                if teacher_top_k_logsumexp_batch is not None:
                    output_teacher_top_k_logsumexp.append(teacher_top_k_logsumexp_batch)
                if forward_teacher_top_k_ids_batch is not None:
                    output_forward_teacher_top_k_ids.append(forward_teacher_top_k_ids_batch)
                if forward_teacher_top_k_logp_batch is not None:
                    output_forward_teacher_top_k_logp.append(forward_teacher_top_k_logp_batch)
            ...
            teacher_logsumexp = None
            if len(output_teacher_logsumexp) > 0:
                teacher_logsumexp = torch.cat(output_teacher_logsumexp, dim=0)
            
            teacher_top_k_logsumexp = None
            if len(output_teacher_top_k_logsumexp) > 0:
                teacher_top_k_logsumexp = torch.cat(output_teacher_top_k_logsumexp, dim=0)
            
            forward_teacher_top_k_ids = None
            if len(output_forward_teacher_top_k_ids) > 0:
                forward_teacher_top_k_ids = torch.cat(output_forward_teacher_top_k_ids, dim=0)

            forward_teacher_top_k_logp = None
            if len(output_forward_teacher_top_k_logp) > 0:
                forward_teacher_top_k_logp = torch.cat(output_forward_teacher_top_k_logp, dim=0)
            ...
            if use_dynamic_bsz:
                ...
                if teacher_logsumexp is not None:
                    teacher_logsumexp = teacher_logsumexp[revert_indices]
                if teacher_top_k_logsumexp is not None:
                    teacher_top_k_logsumexp = (teacher_top_k_logsumexp[revert_indices])
                if forward_teacher_top_k_ids is not None:
                    forward_teacher_top_k_ids = forward_teacher_top_k_ids[revert_indices]
                if forward_teacher_top_k_logp is not None:
                    forward_teacher_top_k_logp = forward_teacher_top_k_logp[revert_indices]
            ...
            if teacher_logsumexp is not None:
                tensors["teacher_logsumexp"] = teacher_logsumexp
            if teacher_top_k_logsumexp is not None:
                tensors["teacher_top_k_logsumexp"] = teacher_top_k_logsumexp
            if forward_teacher_top_k_ids is not None:
                tensors["forward_teacher_top_k_ids"] = forward_teacher_top_k_ids
            if forward_teacher_top_k_logp is not None:
                tensors["forward_teacher_top_k_log_probs"] = forward_teacher_top_k_logp
